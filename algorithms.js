...
//
// `focus` names the one to four pieces the halo rings -- the piece you are
// meant to keep your eye on. Same slot syntax as `keep`, read in the same case
// position, and it has to name pieces `keep` already lights or the ring would
// sit on a greyed-out cubie. It resolves to cubie ids once, so the ring is
// bound to the piece and not to the square of space it started in: when a D
// move carries the corner under the cube, the ring goes with it. `keep` says
// what the case is about; `focus` says what to watch inside that.
//
  ...
  // ---------------------------------------------------------------- Advanced
  {
    ...
    variations: [
      ...
      { label: 'Edge in the equator',
        ...
        focus: ['BR'],
        note: 'The DF edge is jammed sideways in the back-right equator slot, white facing the R face. R2 swings it round to the front-right equator slot — and as a side effect swings the already-solved DR edge up out of the cross to UR. F drops the freed edge straight into DF. The final R2 swings DR back down from UR, finishing the cross.' },
  ...
  },
  {
    ...
    variations: [
      { label: 'Look 1 — A-perm', section: 'Look 1 — place the corners',
        ...
        keep: LAST_LAYER,
        focus: ['ULB', 'URB', 'URF'],
        note: 'Three corners cycle, one stays. Hold the corner that is already home at the front-left.' },
      { label: 'Look 1 — Y-perm',
        moves: ['F', 'R', "U'", "R'", "U'", 'R', 'U', "R'", "F'", 'R', 'U', "R'", "U'", "R'", 'F', 'R', "F'"],
        setup: ['z2', 'y', 'F', "R'", "F'", 'R', 'U', 'R', "U'", "R'", 'F', 'R', "U'", "R'", 'U', 'R', 'U', "R'", "F'"],
        keep: LAST_LAYER,
        focus: ['ULB', 'URF'],
        note: 'Two corners diagonally opposite need swapping — the other two are already home. No two adjacent corners match, so there is nothing to line up first.' },
