...
Start the preview once: `mcp__Claude_Browser__preview_start` with `{"name":"cubeteacher"}` (port 8934). **Append a changing `?nocache=N` on every navigate** — the preview cache served stale code to four agents on the sibling branch. `@media` breakpoints only apply on a *fresh navigate* at the target width; resizing an already-mounted page leaves them stale.
...
**Files:**
- Modify: `CubeTeacher.dc.html:103-112` (tooltip comment references the old class name)
- Modify: `CubeTeacher.dc.html:145-160` (CSS: the track rules)
- Modify: `CubeTeacher.dc.html:308` (template: class name)
- Modify: `CubeTeacher.dc.html:665-676` (`positionRing`)
- Modify: `CubeTeacher.dc.html:695-722` (`followChip`)
- Modify: `CubeTeacher.dc.html:726-733` (`syncFade`)
...
- Consumes: `RING_SLACK` (4) and `SCROLL_EPSILON` (2), already class fields at `CubeTeacher.dc.html:438` and `:441`.
...
Add to the CSS, immediately before the `.ct-strip-until` rule at `CubeTeacher.dc.html:142`:
...
Replace `CubeTeacher.dc.html:145-160` — the four comment lines and the seven `.ct-strip-track` rules — with:
...
At `CubeTeacher.dc.html:308`, change `class="ct-strip-track"` to `class="ct-strip-stack"`. Leave every other attribute on that element exactly as it is (`data-fade`, `ref`, `onScroll`, `onPointerOver`, `onPointerOut`).

In the tooltip comment at `CubeTeacher.dc.html:103-110`, replace the two references to `.ct-strip-track` with `.ct-strip-stack`, and change `(overflow-x:auto computes overflow-y to auto too, CSS Overflow 3 3.3)` to `(overflow-y:auto computes overflow-x to auto too, CSS Overflow 3 3.3)`. The trap is the same in both directions and the comment must describe the axis actually in use.

- [ ] **Step 5: Rotate `positionRing` to Y**

At `CubeTeacher.dc.html:675`, the transform line currently reads:
...
Replace the body of `followChip` at `CubeTeacher.dc.html:695-722` with:
...
Replace the body of `syncFade` at `CubeTeacher.dc.html:726-733` with:
...
**Files:**
- Modify: `CubeTeacher.dc.html:432-433` (state object — add `stripExpanded` and `stackOverflows`)
- Modify: `CubeTeacher.dc.html:478-483` (`componentDidUpdate`)
- Modify: `CubeTeacher.dc.html` `selectAlg` / `selectVar` (reset on case change)
- Modify: `CubeTeacher.dc.html` CSS (add `.ct-stack-toggle` beside `.ct-strip-until`)
- Modify: `CubeTeacher.dc.html:305-341` (template — the toggle button, and `data-expanded` on the stack)
- Modify: `CubeTeacher.dc.html` `render()` return object
...
In the `state = { … }` initialiser at `CubeTeacher.dc.html:432-433`, add both fields to the existing last line:
...
In `componentDidUpdate` at `CubeTeacher.dc.html:478-483`, add the call after `this.syncFade();`:
...
At `CubeTeacher.dc.html:308`, add `data-expanded="{{ stackExpanded }}"` to the `.ct-strip-stack` element.

Immediately after the `.ct-strip-tip` span at `CubeTeacher.dc.html:341`, add:
...
**Files:**
- Modify: `CubeTeacher.dc.html:221` (`--peek`)
- Modify: `CubeTeacher.dc.html:362` (the handle's inline `height`)
...
At `CubeTeacher.dc.html:362`, in the `.ct-sheet-handle` inline style, change `height:32px` to `height:20px`. Leave `padding-top:10px` — that is the grabber's inset from the sheet's top edge and is doing work. 10 above + 4 of grabber + 6 below = 20.

- [ ] **Step 2: Track it in `--peek`**

At `CubeTeacher.dc.html:221`, change:
